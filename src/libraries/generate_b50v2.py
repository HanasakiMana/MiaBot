...
class GenerateB50(object):
    ...
    def get_avatar(self):
        ...
        if self.qq == None:
            ...
            try:
                resp = requests.get(f"https://ssl.ptlogin2.qq.com/getface?imgtype=4&uin={self.qq}",timeout=3).text # 向QQ的api网站发送请求
    ...
    async def generate(self):
        ...
        if self.b50 is not True:
            output = output.resize((1500, 1500))
        ...
        userInfo = userInfo.resize(self.scale(userInfo.size, 1.2))
        output.paste(userInfo, (30, 30))
        ...
        for i in range(0, len(b35List)):
            b35Position.append([init_width + (i%5) * 290, init_heigh + int(i/5) * 150])
        b15Position = []
        if self.b50:
            init_heigh = 1300
        else:
        ...
        for i in range(0, len(b15List)):
            b15Position.append([init_width + (i%5) * 290, init_heigh + int(i/5) * 150])
        
        # 按照难度生成不同曲子的底板
        backGround = [None, None, None, None, None]
        for i in range(0, 5):
            backGround[i] = Image.open(MBase_path + f"/UI_TST_PlateMask.png").convert('RGBA').resize((270, 130)) # 加载背景所需的文件作为蒙版
            backGround[i].paste(Image.new('RGB', (270, 130), (32, 63, 121)), (0, 0), mask=backGround[i]) # 底色
            backGround[i].paste(Image.new('RGB', (270, 2), (255, 255, 255)), (0, 80), mask=backGround[i].crop((0, 79, 270, 81))) # 分割线
        level_color_list = [(107, 188, 69), (245, 187, 65), (238, 130, 125), (147, 88, 212), (210, 172, 249)] # 难度对应的底色
        for i in range(0, 5):
            backGround[i].paste(Image.new(mode='RGB', size=(270, 48), color=level_color_list[i]), (0, 82), mask=backGround[i].crop((0, 82, 270, 130)))
        ...
        for i in range(0, 5):
            ...
            jacketFrame.append(Image.open(MBase_path + f"/UI_UPE_MusicJacket_Base_{diff_level_text}.png").resize((142, 130)))
        
        # dx/sd图标
        dxIcon = Image.open(b50_resource + '/UI_TST_Infoicon_DeluxeMode.png').convert('RGBA')
        dxIcon = dxIcon.resize(self.scale(dxIcon.size, 0.3))
        stdIcon = Image.open(b50_resource + '/UI_TST_Infoicon_StandardMode.png').convert('RGBA')
        stdIcon = stdIcon.resize(self.scale(stdIcon.size, 0.3))
        ...
        for fileName in rank_list:
            ...
            img = img.resize(self.scale(img.size, 0.18))
        ...
        def single_paste(dict: dict, position: list, count: int):
            ...
            jacket = jacket.resize([97, 97])
            output.paste(jacket, (x + 3, y + 17))

            # 编号
            count_img = self.text_to_image(f"# {count}", 15, font='/adobe_simhei.otf')
            output.paste(count_img, (105 + x, 1 + y), mask=count_img)

            # dx/sd图标
            if type == 'DX':
                output.paste(dxIcon, (198 + x, 10 + y), mask=dxIcon)
            elif type == 'SD':
                output.paste(stdIcon, (198 + x, 10 + y), mask=stdIcon)
            
            # 曲名
            title_image = self.text_to_image(name, 13, font='/adobe_simhei.otf',max_length=145)
            output.paste(title_image, (105 + x, 22 + y), mask=title_image)
            ...
            for i in range(len(score)):
                ...
                if score[i] == '.': # 这里是为了下移小数点的位置
                    achi.paste(num, (i * achi_width - 20, 13), mask=num)
                    decimal_flag = True
                else:
                    if not decimal_flag:
                        achi.paste(num, (i * achi_width, 0))
                    else:
                        achi.paste(num, (int((i + 0.2) * (achi_width - 10)), 11))
            achi = achi.resize(self.scale(achi.size, 0.45))
            output.paste(achi, (115 + x, 50 + y), mask=achi)
            ...
            for i in range(len(rank)):
                if rank[i] != 'p':
                    ...
                    rank_new += rank[i]
            output.paste(rankDict.get(rank_new), (205 + x, 82 + y), mask=rankDict.get(rank_new))
            
            # 详细信息
            music_rating = DataProcess(dict, b50=self.b50).get_music_rating(dict)
            info_image = self.text_to_image(f"{ds}->{music_rating}", 15, font='/msyhbd.ttc', max_length=300)
            output.paste(info_image, (105 + x, 72 + y), mask=info_image)

            # 歌曲id
            id_img = self.text_to_image(f"id: {id}", 15, font='/msyhbd.ttc')
            output.paste(id_img, (105 + x, 90 + y), mask=id_img)

            # fc/fs图标
            try:
                output.paste(fcfs_img.get(fc), (202 + x, 100 + y), mask=fcfs_img.get(fc))
                output.paste(fcfs_img.get(fs), (225 + x, 100 + y), mask=fcfs_img.get(fs))
            except:
                pass
